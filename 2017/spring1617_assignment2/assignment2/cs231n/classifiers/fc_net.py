...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_hidden = affine_relu_forward(X, W1, b1)
        scores, cache_out = affine_forward(scores, W2, b2)
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
        ...
        dscores, dW2, db2 = affine_backward(dscores, cache_out)
        dscores, dW1, db1 = affine_relu_backward(dscores, cache_hidden)
...
#'''
    def __init__2(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
                 dropout=0, use_batchnorm=False, reg=0.0,
                 weight_scale=1e-2, dtype=np.float32, seed=None):
        """
        Initialize a new FullyConnectedNet.
        ...
        for i in range(1, self.num_layers + 1):    
            W = weight_scale * \
                np.random.randn(layer_dims[i - 1], layer_dims[i])
...
#'''
    ...
    def loss(self, X, y=None):
        ...
        list_db = {#key[1:]: val for key, val in hidden.iteritems() 
                   key[1:]: val for key, val in hidden.items() 
                   if key[:2] =='db'}
        # Parameters gamma
        list_dgamma = {#key[1:]: val for key, val in hidden.iteritems() 
                       key[1:]: val for key, val in hidden.items() 
                       if key[:6] == 'dgamma'}
        # Paramters beta
        list_dbeta = {#key[1:]: val for key, val in hidden.iteritems() 
                      key[1:]: val for key, val in hidden.items() 
                      if key[:5] == 'dbeta'}
        ...
        for i in range(1, self.num_layers + 1):
            ...
            if is_hidden_layer(i, self.num_layers):
                if self.use_batchnorm:
                    ...
                    scores, batchnorm_c = batchnorm_forward(
                        scores, gamma, beta, bn_p)
                ...
                if self.use_dropout:
                    scores, dropout_c = dropout_forward(
